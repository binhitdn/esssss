...
class LeaderboardSkin(Skin):
    _env = {
        'scale': 2  # General size scale to match background resolution - đưa về như cũ
    }
...
class LeaderboardPage(Layout):
    ...
    def _draw_first_page(self) -> Image:
        ...
        draw.text(
            (text_x, text_y),
            self.skin.top_hours_text.format(
                HH=first_entry.time // 3600,
                MM=(first_entry.time % 3600) // 60,
                SS=first_entry.time % 60,
                amount=first_entry.time // multip
            ),
        ...
        if len(self.entries) >= 2:
            ...
            draw.text(
                (text_x, text_y),
                self.skin.top_hours_text.format(
                    HH=second_entry.time // 3600,
                    MM=(second_entry.time % 3600) // 60,
                    SS=second_entry.time % 60,
                    amount=second_entry.time // multip
                ),
                font=self.skin.top_hours_font,
                fill=self.skin.top_hours_colour,
                anchor='mt'
        ...
        if len(self.entries) >= 3:
            ...
            draw.text(
                (text_x, text_y),
                self.skin.top_hours_text.format(
                    HH=third_entry.time // 3600,
                    MM=(third_entry.time % 3600) // 60,
                    SS=third_entry.time % 60,
                    amount=third_entry.time // multip
                ),
                font=self.skin.top_hours_font,
                fill=self.skin.top_hours_colour,
                anchor='mt'
